...
tm_staticbox_slide_sticky();	
		...
		jQuery('.themetechmount-element-teambox-team-style-four .themetechmount-boxes-row-wrapper .tm-box-col-wrapper:first').addClass("active");
		 jQuery('.themetechmount-element-teambox-team-style-four .themetechmount-boxes-row-wrapper .tm-box-col-wrapper').hover(function(){
		 	jQuery('.themetechmount-element-teambox-team-style-four .themetechmount-boxes-row-wrapper .tm-box-col-wrapper').removeClass("active");
			jQuery(this).addClass("active");
		 });
...
function tm_staticbox_slide_sticky() {
	var ttm_var = jQuery('.tm-static-servicebox-style1 .tm-stepbox-content');
    if (!ttm_var.length) {
        return;
    }
	ScrollTrigger.matchMedia({
		"(min-width: 992px)": function() {
			...
			let ttm_slide = gsap.utils.toArray(".tm-static-servicebox-style1 .tm-stepbox-content");
			const spacer = 30;
		
			let tm_slide_height = ttm_slide[0].offsetHeight;
			ttm_slide.forEach((ttmslide, i) => {
			ScrollTrigger.create({
				trigger: ttmslide, 
				start: () => "top 17%", 
				endTrigger: '.tm-static-servicebox-style1', 
				end: `bottom top+=${tm_slide_height + (ttm_slide.length * spacer)}`,
				pin: true, 
				pinSpacing: false, 
				scrub: true,
			});
		...
		"(max-width:992px)": function() {
			ScrollTrigger.getAll().forEach(ttm_slide => ttm_slide.kill(true));
		}
	});
 }
